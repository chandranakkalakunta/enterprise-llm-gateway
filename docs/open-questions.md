...
| 2 | How should **semantic cache invalidation** work (time TTL, policy version, embedding drift, explicit purge), and how do we guarantee **no cross-ACL / cross-purpose privacy leaks**? | Eng + Security | **Decided (mechanisms)** | **TTL + manual admin + source-document** invalidation; scope at least purpose + sensitivity; DLP-clean only ([ADR-005](adr/005-semantic-cache.md)). Embedding-drift curves and exact per-purpose thresholds remain an **implementation spike**. |
...
| 5 | Do target customers require **multi-region active-active** for the control plane in v1, or is single-region HA + DR enough? | Product + Platform | **Resolved (v1)** | **Single-region HA** is the v1 target ([ADR-009](adr/009-deployment-topology.md)). Multi-region / active-active remains a later extension; keep keys and snapshots location-agnostic. |
...
| 8 | Should purpose be **client-declared only**, or do we invest in **automatic purpose classification** early? | Product | **Resolved** | Purpose is **optional**. Missing purpose → small/fast LLM auto-classify against the catalogue, else **`General`** ([ADR-002](adr/002-policy-engine.md)). Classifier quality remains a residual (§14.5). |
| 9 | How much **raw prompt retention** do security teams demand for forensics vs privacy commitments to employees? | Security + Legal + HR | Open (defaults locked) | **Default: no raw prompts** in metering or logs ([ADR-006](adr/006-metering-and-feedback.md), [ADR-007](adr/007-observability.md)); break-glass is time-limited and audited. Legal/HR demand for richer forensics is still a product conversation. |
